...
<div x-data="{ 
    ...
    loading: false,
    isMobile: {{ $isMobileApp ? 'true' : 'false' }},
    ...
    <div class="flex items-center w-full gap-2 text-[10px] divider opacity-60 my-3 py-0 leading-none text-gray-500">
        <div class="grow border-t border-current opacity-30"></div>
        <span class="px-2 font-medium tracking-widest uppercase">{{ __('Atau') }}</span>
        <div class="grow border-t border-current opacity-30"></div>
    </div>

    <div class="flex flex-wrap items-center justify-center w-full gap-0 mt-3">
        <x-filament::button tag="button" type="button" color="gray" outlined
            class="flex-1 w-full transition-all duration-100 transform shadow-sm"
            x-bind:class="!(agreed && remembered) ? 'opacity-40 cursor-not-allowed' : 'active:scale-[0.98] hover:shadow-md'"
            x-on:click="
                if (!(agreed && remembered)) return;
                loading = true;
                if (isMobile) {
                    fetch(googleUrl + '?native=1', {
                        method: 'GET',
                        headers: { 'X-NativePHP': '1', 'X-Requested-With': 'XMLHttpRequest' }
            ...
            <div class="flex items-center justify-center gap-3">
                ...
                <span x-text="loading ? '{{ __('Menghubungkan...') }}' : '{{ __('Masuk Dengan Google') }}'"></span>
            </div>
        </x-filament::button>
    </div>
    ...
    <div class="w-full text-center mt-4">
        @if (request()->routeIs('filament.user.auth.login'))
            <p class="text-[13px] text-gray-500 font-medium">
                {{ __('Belum punya akun?') }}
                <x-filament::link :href="route('filament.user.auth.register')" color="primary"
        ...
        @elseif (request()->routeIs('filament.user.auth.register'))
            <p class="text-[13px] text-gray-500 font-medium">
                {{ __('Sudah punya akun?') }}
                <x-filament::link :href="route('filament.user.auth.login')" color="primary"
                    class="text-[13px] font-bold hover:underline focus:underline active:underline transition-all">
                    {{ __('Masuk') }}
                </x-filament::link>
            </p>

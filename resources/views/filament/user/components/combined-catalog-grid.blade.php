@php
    $wo = (isset($records) && $records->isNotEmpty()) ? $records->first() : \App\Models\WeddingOrganizer::first();
    
    if ($wo) {
        // Load packages and products
        $packages = $wo->packages()->with(['category'])->get()->map(function($item) {
            $item->catalog_type = 'package';
            return $item;
        });
        
        $products = $wo->products()->where('is_active', true)->with(['category'])->get()->map(function($item) {
            $item->catalog_type = 'product';
            return $item;
        });
        
        $items = $packages->concat($products);
    } else {
        $items = collect();
    }

@php
    ...
    $firstWoId = cache()->remember('header_first_wo_id', 300, fn() =>
        \App\Models\WeddingOrganizer::value('id') ?? 1
    );
@endphp
<header
    ...
    <nav class="flex items-center {{ $isMobile ? 'gap-2' : 'gap-2 lg:gap-3' }}">
        ...
        @if(!$isMobile)
            <div class="hidden lg:flex items-center gap-2 lg:gap-3">
                @auth
                    <a href="{{ \App\Providers\NativeServiceProvider::normalizeUrl(route('filament.user.resources.home.index', ['record' => $firstWoId])) }}"
